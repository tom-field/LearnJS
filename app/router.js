...
module.exports = app => {
    ...
    const prefix = '/cnode-ws';

    const userRequired = middleware.userRequired();

    router.get('/cnode-ws/auth', controller.auth.index);
    ...
    app.passport.mount('github',{
        loginURL: '/cnode-ws/passport/github',
        callbackURL: '/cnode-ws/passport/github/callback',
        successReturnToOrRedirect: '/cnode-ws/auth',
    });
    app.passport.mount('google', {
        scope: ['email', 'profile', 'openid'],
        loginURL: '/cnode-ws/passport/google',
        callbackURL: '/cnode-ws/passport/google/callback',
        successReturnToOrRedirect: '/cnode-ws/auth',
    });
    ...
    router.post(prefix + '/user/top100', controller.user.top100);                    // 显示积分前一百用户页
    ...
    router.post(prefix + '/topic/list', userRequired, controller.topic.list);                      // 获取话题列表
